...
# Dictionary of monomers with [*] as attachment points
monomer_smiles = {
...
# Sequences to be converted to SMILES
sequences = [
    "khvorovalab_nomenclature_seq_here1",
    "khvorovalab_nomenclature_seq_here2",
    "khvorovalab_nomenclature_seq_here3",
    "khvorovalab_nomenclature_seq_here4",
    "khvorovalab_nomenclature_seq_here5",
...
# SMILES GENERATING LOOP BELOW
for i, (name,seq) in enumerate(zip(name,sequences), 1):
    print(f"\n--- Sequence {i} ({name}): {seq} ---")
    sequence = re.findall(r'\([a-zA-Z]+\)#?', seq)
    ...
    try:
        ...
        smiles = Chem.MolToSmiles(rna_mol, isomericSmiles=True, canonical=True) # <- SMILES generating line
        sequence_id = i

        data_rows.append({
            "sequence_id": i,
            "name": name,
            "SMILES": f'"{smiles}",',    # <- adds quotes and comma on smiles in excel
        })
...
df.to_excel('rna_smiles_with_names.xlsx', index=False)
